...
impl Workspace {
    ...
    /// Adjust the adjacent column after a border-drag resize.
    ///
    /// `old_focused_width` is the focused column's width before the drag. The
    /// focused column has already been resized by the caller; this method moves
    /// the boundary by the opposite amount on the adjacent column, so two
    /// neighboring columns keep a stable combined width. `left_edge` selects
    /// the left neighbor (`true`) or the right neighbor (`false`).
